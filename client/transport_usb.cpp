...
static int UsbReadPayload(usb_handle* h, apacket* p) {
	...
	D("UsbReadPayload(): bad length!  command: %u arg0: %u arg1: %u len: %u check %u magic: 0x%x (0x%x)", p->msg.command, p->msg.arg0, p->msg.arg1, p->msg.data_length, p->msg.data_check, p->msg.magic, p->msg.command^p->msg.data_length);
...
static int remote_read(apacket* p, usb_handle* usb) {
    ...
    if (p->msg.data_length) {
        ...
        if (n < 0) {
            D("remote usb: terminated (data) expected: %u", p->msg.data_length);
            return -1;
        }
